...
const books = [
  ...
  },
  {
    ...
    eduzzLink: "https://sun.eduzz.com/60E2DVJDW3",
  ...
  },
  {
    ...
    priceNum: 33.90,
    color: accentColors[4],
    coverImg: "imagens/Leitura e Escrita em Ação.png",
  ...
  {
    id: 22,
    title: "Declaração de Imposto de Renda? Nem pensar! Faça sua declaração com quem entende.- Declaração de Imposto de Renda",
    ...
    desc: "Não erre na declaração do IR e evite a malha fina! Análise completa, preenchimento correto, identificação de restituições e entrega dentro do prazo. Atendimento pelo WhatsApp (63) 999596068.",
...
function openModal(book) {
  ...
  const eduzzSection = document.getElementById('kirvanoPaymentSection');
  const btn = document.getElementById('kirvanoBuyBtn');
  if (book.eduzzLink) {
    if (pixSection) pixSection.style.display = 'none';
    if (eduzzSection) eduzzSection.style.display = 'block';
    if (btn) btn.href = book.eduzzLink;
  } else {
    if (pixSection) pixSection.style.display = 'block';
    if (eduzzSection) eduzzSection.style.display = 'none';
    const pixKeyEl = document.getElementById('pixKeyText');
    const qrBoxEl = document.getElementById('qrBox');
    if (pixKeyEl) pixKeyEl.textContent = book.pixKey || '';
    if (qrBoxEl) qrBoxEl.innerHTML = generateQRSVG((book.pixKey || '') + (book.price || ''));
  }
